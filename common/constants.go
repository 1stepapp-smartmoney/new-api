...
const (
	...
	// PlatformRequestIdKey is the gin context key holding the caller-supplied
	// reconciliation correlation id (fork §10). It is distinct from
	// RequestIdKey (generated by this gateway) and UpstreamRequestIdKey
	// (returned by the upstream provider).
	PlatformRequestIdKey = "X-Platform-Request-Id-Value"
)

// PlatformRequestIdHeader is the inbound header carrying the caller's
// reconciliation correlation id. The reconciliation spec lets both sides agree
// on the header name offline, so it is configurable; an empty value disables
// capture entirely.
var PlatformRequestIdHeader = "X-Platform-Request-Id"

// MaxPlatformRequestIdLength bounds the stored correlation id. The value is
// caller-controlled, and the column is varchar(128).
const MaxPlatformRequestIdLength = 128
...
var (
	...
	// Reconciliation endpoints get their own bucket: the spec requires the
	// reconciliation key's throttling to be independent of the AI-calling keys.
