...
export function CommonLogsFilterBar<TData>(
  ...
  // fork §4: the IP filter is root-only; upstream's isRootView replaces the
  // local useIsRoot hook so the filter and the IP column gate identically.
  ...
  const searchState = useMemo<CommonLogDraft>(() => {
    ...
    const filters: CommonLogFilters = {
      ...
      ip: searchParams.ip || undefined, // fork §4
      isStream: searchParams.isStream || undefined, // fork §7
    }
    return {
    ...
    searchParams.ip, // fork §4
    searchParams.isStream, // fork §7
    searchParams.type,
  ])
  ...
  const hasExpandedFilters =
    ...
    (isRoot && !!filters.ip) || // fork §4
    !!filters.isStream // fork §7
  ...
  const expandedFilterCount = [
    ...
    filters.requestId,
    filters.upstreamRequestId,
    isRoot ? filters.ip : undefined, // fork §4
    filters.isStream, // fork §7
  ...
  const advancedFilters = (
    <>
      ...
      {/* fork §7: stream / non-stream filter */}
      <LogsFilterField>
        <Select
          items={[
          ...
          value={
            filters.isStream && filters.isStream !== ''
              ? filters.isStream
              : '__all__'
          }
          onValueChange={(v) =>
