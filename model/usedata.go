...
// fork §1: each of these takes an optional modelName filter for the dashboard
// model-name filter. Upstream's aggregated Select/Group is preserved; the fork
// only inserts a conditional `model_name = ?` WHERE when modelName != "".
func GetQuotaDataByUsername(username string, startTime int64, endTime int64, modelName string) (quotaData []*QuotaData, err error) {
	...
	err = tx.Group("user_id, username, model_name, created_at").
...
// GetDistinctModelNames 返回 quota_data 中出现过的模型名列表，用于数据看板筛选下拉框。
// 管理员返回全局列表；普通用户仅返回自己产生过数据的模型。 (fork §1)
func GetDistinctModelNames(userId int, isAdmin bool) ([]string, error) {
	tx := DB.Table("quota_data").Select("DISTINCT model_name")
	if !isAdmin {
		tx = tx.Where("user_id = ?", userId)
	}
	var names []string
	err := tx.Where("model_name <> ''").Order("model_name ASC").Pluck("model_name", &names).Error
	return names, err
}
