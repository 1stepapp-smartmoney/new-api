...
import { GitBranch, Sparkles, KeyRound, Unplug } from 'lucide-react'
...
export function useCommonLogsColumns(
  isAdmin: boolean,
  isRoot: boolean // fork §4: gates the root-only IP column
): ColumnDef<UsageLog>[] {
  ...
  // fork §4: root-only IP column. Backend strips Log.Ip for non-root callers,
  // so even a dev-tools-unhidden column renders empty for non-root.
  if (isRoot) {
    columns.push({
      id: 'ip',
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='flex items-center gap-1.5'>
            ...
            />
            {/* fork §9: client-disconnect indicator, independent of the
                stream-error mark StreamTpsCell already renders. Stream case via
                stream_status.client_gone; non-stream via client_disconnected. */}
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <div className='space-y-0.5 text-xs'>
                      <p className='font-medium'>{t('Client Disconnected')}</p>
                      ...
                      {other?.stream_status?.client_gone_at_ms != null && (
                        <p>
                          {t('Disconnect at')}:{' '}
                          {(other.stream_status.client_gone_at_ms / 1000).toFixed(
                            2
                          )}
                          s
                        </p>
                      ...
                      {!log.is_stream &&
                        other?.client_disconnected_at_ms != null && (
                          <p>
                            {t('Disconnect at')}:{' '}
                            {(other.client_disconnected_at_ms / 1000).toFixed(2)}
                            s
                          </p>
                        )}
                    </div>
