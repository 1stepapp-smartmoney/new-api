...
interface DetailsDialogProps {
  log: UsageLog
  isAdmin: boolean
  isRoot: boolean // fork §4: gates IP visibility
  open: boolean
...
export function DetailsDialog(props: DetailsDialogProps) {
  ...
  // fork §4: IP is operator-only. Backend strips `ip` for non-root callers, so
  // this client-side root guard is defence-in-depth.
  ...
  return (
    ...
    >
      <div className='w-full max-w-full min-w-0 space-y-2.5 overflow-x-hidden py-1 sm:space-y-3'>
        ...
        {/* Stream status details (admin only). fork §9: show whenever
            stream_status exists AND either the stream ended abnormally
            (status !== 'ok') OR the client disconnected (client_gone) — so the
            truncation point is visible even on a cleanly-ended stream. */}
        {other?.stream_status &&
          (other.stream_status.status !== 'ok' ||
            ...
            >
              ...
              />
              {/* fork §9: client-disconnect truncation detail */}
              {other.stream_status.client_gone && (
                <>
                  <DetailRow
                  ...
                  {other.stream_status.client_gone_at_chunks != null && (
                    <DetailRow
                      ...
                      value={String(other.stream_status.client_gone_at_chunks)}
                      mono
                    />
                  )}
        ...
        {/* fork §9: non-stream client disconnect. Streams use stream_status
            above; this surfaces the same signal for non-stream requests where
            the client TCP-closed before we finished talking to upstream. */}
        {props.isAdmin &&
          !props.log.is_stream &&
          other?.client_disconnected && (
            <DetailSection label={t('Client Disconnect')}>
              ...
              {other.client_disconnected_at_ms != null && (
                <DetailRow
                  label={t('Disconnect at')}
                  value={formatUseTime(other.client_disconnected_at_ms / 1000)}
                  mono
                />
              )}
            </DetailSection>
