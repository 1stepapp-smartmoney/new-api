    ...
    >
      <ScrollArea className='max-h-[70vh] min-w-0 overflow-hidden pr-2 max-sm:max-h-[calc(100dvh-7rem)] sm:pr-4'>
        <div className='w-full max-w-full min-w-0 space-y-2.5 overflow-hidden py-1 sm:space-y-3'>
          ...
          {/* Stream status details (admin only). Show whenever stream_status
              exists AND either:
                - the stream ended abnormally (status !== 'ok'), or
                - the downstream client disconnected mid-stream (client_gone).
              Surfacing client_gone independent of status lets operators see the
              truncation point even when the stream itself ended cleanly. */}
          {props.isAdmin &&
            other?.stream_status &&
